...
export class ProfileService {
  async list(userId: string) {
    // 1) profiles 전체 조회 (Firestore orderBy)
    ...
    const petIds = [...new Set(profilesSnap.docs.map(d => d.data().petId))];
    const profileIds = profilesSnap.docs.map(d => d.id);

    // 2) pets(getAll) + baseVideos + motions 병렬 조회
    const petRefs = petIds.map(id => db.collection('pets').doc(id));
    const [petDocs, baseVideoDocs, motionDocs] = await Promise.all([
      petRefs.length > 0 ? db.getAll(...petRefs) : Promise.resolve([]),
    ...
    const petMap = new Map(
      petDocs.filter(d => d.exists).map(d => [d.id, d.data()!])
    );
    ...
    return profilesSnap.docs.map(doc => {
      const data = doc.data();
      const petData = petMap.get(data.petId);
      const pet = petData ? { id: data.petId, name: petData.name, emoji: petData.emoji } : null;

      const baseVideos = (baseVideosByProfileId.get(doc.id) || []).map(v => ({
        id: v.id,
      ...
      const motions = (motionsByProfileId.get(doc.id) || []).map(m => ({
        id: m.id,
        name: m.data().name,
  ...
  async getById(userId: string, profileId: string) {
    ...
    // 병렬 조회: pet + baseVideos + motions
    const [petDoc, baseVideosSnap, motionsSnap] = await Promise.all([
      db.collection('pets').doc(data.petId).get(),
  ...
  async create(userId: string, data: { petId: string; name: string; type: 'STANDING' | 'SITTING'; selectedMotionTypes?: MotionType[]; creditPrepaid?: boolean }) {
    ...
    const motions = data.selectedMotionTypes || [];
    const name = sanitizeText(data.name);
    const petName = petDoc.data()!.name;

    // 크레딧 차감: creditPrepaid면 베이스 비용 스킵 (이미 Step 1에서 선결제)
    const baseCost = data.creditPrepaid ? 0 : CREDIT_COSTS.BASE_VIDEO_CREATE;
    const totalCost = baseCost + motions.length * CREDIT_COSTS.MOTION_CREATE;
    if (totalCost > 0) {
      const desc = motions.length > 0
        ? `프로필 생성 (모션 ${motions.length}개)`
        : '프로필 생성';
      await creditService.spend(userId, totalCost, desc, 'PROFILE');
    }
  ...
  async listMotions(userId: string, profileId: string) {
    ...
    return motionsSnap.docs.map(doc => {
      const data = doc.data();
      return {
        id: doc.id,
  ...
  async createMotion(userId: string, profileId: string, motionType: MotionType) {
    ...
    // ★ 모션 영상 자동 생성 (fire-and-forget)
    aiService.generateMotionVideo(userId, profileId, motionRef.id).catch((err) => {
      logger.error(`모션 자동 생성 실패 (motion: ${motionRef.id}):`, err);
    });
