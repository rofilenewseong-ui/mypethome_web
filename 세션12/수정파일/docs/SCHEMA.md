...
| petName | string | YES | — | 비정규화된 반려동물 이름 |
| type | enum | YES | — | `STANDING` \| `SITTING` |
| selectedStartFrameUrl | string | NO | null | 선택된 스타트 프레임 이미지 URL |
...
반려동물 AI 메신저 채팅방. pet 등록 시 자동 생성. 비정규화 필드 포함.
...
| userId | string | YES | — | FK → users |
| petId | string | YES | — | FK → pets |
| petName | string | NO | null | 비정규화: 반려동물 이름 |
| petEmoji | string | NO | null | 비정규화: 반려동물 이모지 |
| petFrontPhoto | string | NO | null | 비정규화: 반려동물 정면 사진 URL |
| lastMessageAt | timestamp | YES | serverTimestamp | 마지막 메시지 시각 |
| lastMessageContent | string | NO | null | 비정규화: 마지막 메시지 내용 |
| lastMessageSenderType | enum | NO | null | 비정규화: 마지막 메시지 발신자 (`USER` \| `PET_AI`) |
| createdAt | timestamp | YES | serverTimestamp | 생성일 |

**Queries:**
- `WHERE userId == ? ORDER BY lastMessageAt DESC` (내 채팅방 목록 — composite index)
- `WHERE userId == ? AND petId == ?` (특정 채팅방 조회)

**Denormalization Notes:**
- `petName`, `petEmoji`, `petFrontPhoto`: pet 정보 변경 시 동기화 필요
- `lastMessageContent`, `lastMessageSenderType`: 메시지 발송 시 자동 갱신
...
**Queries:**
- `WHERE chatRoomId == ? ORDER BY createdAt DESC` (메시지 목록, 서버사이드 페이지네이션)
...
**Queries:**
- `WHERE userId == ? ORDER BY createdAt DESC` (내 크레딧 이력, 서버사이드 페이지네이션)
...
**Queries:**
- `WHERE userId == ? ORDER BY deletedAt DESC` (내 휴지통 — composite index)
...
| cafe24OrderId | string | NO | — | Cafe24 주문 ID |
| cafe24ProductId | string | NO | — | Cafe24 상품 ID |
| cafe24BuyerEmail | string | NO | null | Cafe24 구매자 이메일 |
| createdAt | timestamp | YES | serverTimestamp | 생성일 |
...
프론트엔드 이벤트 트래킹. 배치 전송 + 오프라인 큐 지원.

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| id | string | auto | — | Firestore document ID |
| userId | string | YES | — | FK → users |
| sessionId | string | YES | — | 클라이언트 세션 ID |
| event | string | YES | — | 이벤트명 (page_view, login, motion_tap 등) |
| properties | map | YES | `{}` | 이벤트별 추가 데이터 |
| page | string | YES | — | 현재 페이지 경로 |
| deviceInfo | map | YES | — | platform, userAgent, screenWidth, screenHeight, isStandalone |
| createdAt | timestamp | YES | serverTimestamp | 이벤트 시각 |

**Queries:**
- `WHERE event == ? AND createdAt >= ? AND createdAt <= ?` (이벤트별 집계)
- `WHERE userId == ? ORDER BY createdAt DESC` (사용자별 이벤트)

**Estimated rows:** ~1,000,000+ (빠르게 증가)

---

## 15. playbackSessions

홀로그램 재생 세션 추적. 재생 시간, 모션 탭/잠금 등 기록.
...
| profileId | string | YES | — | FK → profiles |
| petId | string | YES | — | FK → pets |
| startedAt | timestamp | YES | — | 재생 시작 시각 |
| endedAt | timestamp | NO | null | 재생 종료 시각 |
| duration | number | YES | `0` | 재생 시간 (초) |
| motionTaps | number | YES | `0` | 모션 탭 횟수 |
| motionLocks | number | YES | `0` | 모션 잠금 횟수 |
| motionDetails | array | YES | `[]` | 모션 상세 (position, motionId, timestamp) |
| deviceInfo | map | YES | — | platform, screenWidth, screenHeight |

**Queries:**
- `WHERE userId == ? ORDER BY startedAt DESC` (사용자별 세션)
- `WHERE startedAt >= ? AND startedAt <= ?` (기간별 집계)
...
일별 서비스 통계. 관리자 대시보드용. 문서 ID = `YYYY-MM-DD`.

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| id | string | manual | — | `YYYY-MM-DD` 형식 |
| totalUsers | number | YES | `0` | 전체 사용자 수 |
| newUsers | number | YES | `0` | 신규 가입자 수 |
| activeUsers | number | YES | `0` | 활성 사용자 수 |
| totalPageViews | number | YES | `0` | 전체 페이지뷰 |
| totalPlaybackMinutes | number | YES | `0` | 전체 재생 시간 (분) |
| avgPlaybackMinutes | number | YES | `0` | 평균 재생 시간 (분) |
| totalMotionTaps | number | YES | `0` | 전체 모션 탭 수 |
| totalMessagesSent | number | YES | `0` | 전체 발신 메시지 수 |
| totalMessagesReceived | number | YES | `0` | 전체 수신 메시지 수 |
| totalCreditsSpent | number | YES | `0` | 전체 크레딧 소비 |
| totalCreditsEarned | number | YES | `0` | 전체 크레딧 획득 |
| newPets | number | YES | `0` | 신규 반려동물 수 |
| newProfiles | number | YES | `0` | 신규 프로필 수 |
| newBaseVideos | number | YES | `0` | 신규 베이스 영상 수 |
| newMotions | number | YES | `0` | 신규 모션 수 |
| topPages | array | YES | `[]` | 인기 페이지 (path, views) |
| updatedAt | timestamp | YES | serverTimestamp | 최종 갱신 시각 |

**Queries:**
- `doc(YYYY-MM-DD)` (특정 일자 조회)
- `ORDER BY updatedAt DESC LIMIT 30` (최근 30일)

**Estimated rows:** ~365/year
...
| chatMessages | chatRoomId ASC, createdAt DESC | 메시지 페이지네이션 |
| creditTransactions | userId ASC, createdAt DESC | 크레딧 이력 페이지네이션 |
